...
def change_signature(group: Optional[str] = None) -> tuple[int, str]:
    """노출 워크플로우 집합의 변경 시그니처 (개수, 최신수정시각).

    MCP 도구 목록 변경 감지용. 노출 토글·그래프 저장·이름/그룹 변경은 updated_at 을 갱신하고,
    생성/삭제/노출해제는 개수를 바꾸므로, (COUNT, MAX(updated_at)) 만으로 변경을 감지할 수 있다.
    group 지정 시 해당 MCP 그룹으로 한정(불필요한 재빌드/알림 방지). 단일 인덱스 없는 가벼운 집계 1쿼리.
    """
    conn = connect()
    try:
        if group:
            row = conn.execute(
                "SELECT COUNT(*) AS c, COALESCE(MAX(updated_at),'') AS m FROM workflows "
                "WHERE mcp_exposed=1 AND COALESCE(mcp_group,'')=?",
                (group,),
            ).fetchone()
        else:
            row = conn.execute(
    ...
    try:
        ...
        for r in rows:
            ...
            api_rows = conn.execute(
                "SELECT DISTINCT c.id AS id, c.name AS name FROM nodes n "
                "JOIN operations o ON n.operation_id = o.id "
                "JOIN specs sp ON o.spec_id = sp.id "
                "JOIN connections c ON sp.connection_id = c.id "
                "WHERE n.workflow_id=? ORDER BY c.id",
                (r["id"],),
            ).fetchall()
            s["apis"] = [{"id": x["id"], "name": x["name"]} for x in api_rows]
            out.append(s)
        return out
    finally:
        conn.close()
...
def _node_row_to_wire(row) -> dict:
    keys = row.keys()
    ro = row["read_only"] if "read_only" in keys else None
    return {
        "id": row["node_key"],
    ...
    try:
        ...
        if nodes is not None:
            ...
            for n in nodes:
                ...
                ro = n.get("read_only")
                conn.execute(
                    "INSERT INTO nodes (workflow_id, node_key, operation_id, type, label, base_url, read_only, params, position_x, position_y) "
                    "VALUES (?,?,?,?,?,?,?,?,?,?)",
                    (
                        workflow_id, n["id"], n.get("operation_id"),
                        n.get("type", "api_call"), n.get("label"), n.get("base_url"),
                        None if ro is None else (1 if ro else 0),
