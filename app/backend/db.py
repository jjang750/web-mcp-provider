...
COLUMN_MIGRATIONS: list[tuple[str, str, str]] = [
    ...
    ("nodes", "base_url", "TEXT"),
    ("nodes", "read_only", "INTEGER"),
    ("edges", "label", "TEXT"),
    ("executions", "source", "TEXT"),
    ("executions", "tool_name", "TEXT"),
    ("specs", "connection_id", "INTEGER"),
]


def _backfill_connections(conn: sqlite3.Connection) -> None:
    """기존 스펙을 연결로 자동 편입(멱등). connection_id 가 비어 있는 스펙마다
    스펙명 기반 연결을 1개씩 생성하고, base_url 은 해당 스펙 오퍼레이션의 base_url 중
    첫 비어있지 않은 값을 채택한다."""
    rows = conn.execute(
        "SELECT id, name FROM specs WHERE connection_id IS NULL ORDER BY id"
    ).fetchall()
    for r in rows:
        bu = conn.execute(
            "SELECT base_url FROM operations WHERE spec_id=? AND base_url IS NOT NULL "
            "AND base_url<>'' ORDER BY id LIMIT 1",
            (r["id"],),
        ).fetchone()
        base_url = bu["base_url"] if bu else None
        cur = conn.execute(
            "INSERT INTO connections (name, base_url, auth_type, auth_config, enabled) "
            "VALUES (?,?,?,?,1)",
            (r["name"] or f"api_{r['id']}", base_url, "none", None),
        )
        conn.execute(
            "UPDATE specs SET connection_id=? WHERE id=?", (cur.lastrowid, r["id"])
        )
