...
async def upload_spec(file: UploadFile = File(...), connection_id: int | None = Form(None)):
    ...
    return specs_repo.create_spec_with_operations(
        name, "file", file.filename, raw, parsed, connection_id=connection_id
    )
...
def spec_from_url(req: FromUrlRequest):
    ...
    return specs_repo.create_spec_with_operations(
        name, "url", req.url, raw, parsed, connection_id=req.connection_id
    )


@router.post("/specs/{spec_id}/resync")
def resync_spec(spec_id: int):
    """source_type='url' 스펙을 원본 URL에서 다시 가져와 오퍼레이션을 갱신."""
    spec = specs_repo.get_spec(spec_id)
    if spec is None:
        raise HTTPException(404, "스펙을 찾을 수 없습니다.")
    if spec.get("source_type") != "url" or not spec.get("source_ref"):
        raise HTTPException(400, "URL로 등록된 스펙만 재동기화할 수 있습니다.")
    url = spec["source_ref"]
    try:
        with httpx.Client(timeout=30.0, follow_redirects=True, trust_env=False) as cli:
            resp = cli.get(url)
        ...
        raise HTTPException(400, f"URL에서 스펙을 가져오지 못했습니다: {exc}")
    try:
        parsed = engine_bridge.parse_openapi(raw, source_hint=url)
    except Exception as exc:
        raise HTTPException(400, f"스펙 파싱 실패: {exc}")
    return specs_repo.resync_operations(spec_id, raw, parsed)
